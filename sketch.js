...
function drawSkeleton() {
  ...
  for (let i = 0; i < poses.length; i++) {
    ...
    if(partA.score > 0.1){  //鏡頭抓取眼睛
      push() //
      image(inup,partA.x-25,partA.y-25,inup.width,inup.height)
      pop()
    }

    if(partB.score > 0.1){
      push()
      image(inup,partB.x-25,partB.y-25,inup.width,inup.height)
      pop()
   }
    //wrist
   ...
   if(partA.score > 0.1){
    ...
    stroke("#fff321")
    strokeWeight(3)
    text("412730284 王沛瑜",partA.x-50,partA.y-50)
    pop()
  }
